"""Shared FFXI lookup tables and utilities."""
...
ITEM_CATEGORIES = {
    "general": lambda id_: id_ <= 0x0FFF or (0x2200 <= id_ < 0x2800) or id_ >= 0xF200,
    "usable": lambda id_: 0x1000 <= id_ < 0x2000,
    "puppet": lambda id_: 0x2000 <= id_ < 0x2200,
    "armor": lambda id_: (0x2800 <= id_ < 0x4000) or (0x5A00 <= id_ < 0x7000),
    "weapon": lambda id_: 0x4000 <= id_ < 0x5A00,
    "slip": lambda id_: 0x7000 <= id_ < 0x7400,
    "instinct": lambda id_: 0x7400 <= id_ < 0x7800,
    "monipulator": lambda id_: 0x7800 <= id_ < 0xF200,
}


def decode_bitmask(bitmask: int, names: dict, start: int, end: int) -> list[str]:
    return [names[bit] for bit in range(start, end) if bitmask & (1 << bit) and bit in names]
